 ...
 *
 * Partition of the proven XIAO nRF52840 Sense firmware (GaitIMU v11b) / of the
 * ESP32-C3 SuperMini port (esp32_gait). Identical BLE protocol & UUIDs, so the
 * same index.html works as-is (with the gyro scaling constant 0.0305).
 *
 * Differences vs. the SuperMini port (hardware only):
 *   - I2C on the XIAO's SDA/SCL pins: SDA = D4 (GPIO6), SCL = D5 (GPIO7).
 *     Note: these are the same pins as the UART TX/RX — keep the serial port
 *     unused while streaming.
 *   - Status LED disabled by default: the XIAO's onboard LED is a WS2812
 *     NeoPixel on GPIO8 (needs a NeoPixel library), not a plain GPIO LED.
 *     Set MODULE_LED to any free GPIO if you attach an external LED.
 *   - MPU-6050 on Wire: DLPF enabled (1 kHz) + SMPLRT_DIV=4  ->  200.00 Hz
 *   - 4-sample batched notifications (50/s), ±8 g / ±1000 dps
 *   - ID(read) ; CMD(write: 01 start, 02 stop, 03 [n] samples/packet)
 *     TIME(write+notify) ; DATA(notify)
 *   - Clock sync: browser writes a 6-byte Unix epoch, we latch millis() and
 *     ACK a 4-byte relative time.
 *   - Battery service intentionally omitted: the XIAO ESP32-C3 has no battery
 *     sense pin, so index.html shows "–" for battery on each card.
 *
 * DATA packet: [seq][N][tFirst u32 LE] + N×12 bytes (ax ay az gx gy gz int16 LE)
 * Scaling (index.html): ±8 g -> g = raw × 0.000244 ;  ±1000 dps -> dps = raw × 0.0305
 *
 * Wiring (MPU-6050 -> XIAO ESP32-C3):
 *   VCC -> 3V3     (MPU-6050 is 3.3 V ONLY — never 5 V)
 *   GND -> GND
 *   SDA -> D4 (GPIO6)   (board "SDA" label)
 *   SCL -> D5 (GPIO7)   (board "SCL" label)
 *   AD0 -> GND     (I2C address 0x68)
 *   INT -> unused
 * Most MPU-6050 breakouts already carry 4.7 kΩ pull-ups on SDA/SCL; add them
 * on both lines if yours does not, or this firmware will show I2C errors.
 * ==========================================================================*/
...
uint32_t statLast = 0, pktCount = 0, smpCount = 0, i2cErrors = 0, i2cFailStreak = 0;

/* ══ I2C helpers (raw MPU-6050 access — no third-party library needed) ══ */
...
static uint8_t readReg(uint8_t reg) {
  ...
  uint8_t v = 0xFF;
  if (Wire.available() >= 1) v = (uint8_t)Wire.read();
...
/* ══ packet build / send (ESP32 notifies explicitly, unlike ArduinoBLE) ══ */
  ...
  void onWrite(BLECharacteristic* c) override {
    ...
    switch ((uint8_t)v[0]) {
      ...
      case CMD_STOP:
        ...
        break;
      case CMD_SET_SPP: {
        uint8_t n = v.size() >= 2 ? (uint8_t)v[1] : 0;
        flushPacket();
...
class GaitTimeCallbacks : public BLECharacteristicCallbacks {
  void onWrite(BLECharacteristic* c) override {
    std::string v = c->getValue();
    if (v.size() != 6) return;            /* 6-byte probe or real epoch      */
    uint32_t sec; uint16_t ms10;
    memcpy(&sec,  v.data(),     4);
    memcpy(&ms10, v.data() + 4, 2);
    syncMillis = millis();
...
void setup() {
  ...
  timeChar->addDescriptor(new BLE2902());   /* client subscription control */
